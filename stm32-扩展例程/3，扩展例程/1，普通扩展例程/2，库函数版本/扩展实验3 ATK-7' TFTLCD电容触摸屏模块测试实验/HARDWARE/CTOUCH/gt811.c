...
u8 GT811_Init(void)
{
  ...
  GPIO_InitTypeDef  GPIO_InitStructure; 
	u16 version=0;
	u8 temp;
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOF|RCC_APB2Periph_AFIO, ENABLE);	 //使能AFIO,PF端口时钟

  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11;				 // PF.11 端口配置
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 //IO口速度为50MHz
  GPIO_Init(GPIOF, &GPIO_InitStructure);					 //根据设定参数初始化GPIOF11
  GPIO_SetBits(GPIOF,GPIO_Pin_11);						 //PF.11输出高

  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10;				 // PF10 端口配置
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; 		 //上拉输入
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;		 //IO口速度为50MHz
  GPIO_Init(GPIOF, &GPIO_InitStructure);					 //根据设定参数初始化GPIOF10
  GPIO_SetBits(GPIOF,GPIO_Pin_10);						 //PF10输出高
