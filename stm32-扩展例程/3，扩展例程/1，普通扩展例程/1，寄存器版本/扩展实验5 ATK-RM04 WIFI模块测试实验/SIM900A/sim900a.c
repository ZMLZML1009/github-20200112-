...
{ 
	...
	Show_Str(x,y+100,200,16,"KEY2/KEY_UP:返回上级菜单",16,0);
}
//sim900a短信测试
//用于读短信或者发短信
//返回值:0,正常
...
u8 sim900a_sms_test(void)
{
	...
	while(1)
	{
		...
		{ 
			...
			sim900a_sms_ui(40,30);
			timex=0;			
		}else if(key==WKUP_PRES||key==KEY2_PRES)break;
		timex++;
		if(timex==20)
		{
			timex=0;
...
{ 
	...
	Show_Str(30,50,200,16,"KEY2/KEY_UP:退出  KEY0:发送数据",12,0); 	
	...
	{ 
		key=KEY_Scan(0);
		if(key==WKUP_PRES||key==KEY2_PRES)//退出测试		 
		{  
...
void sim900a_gprs_ui(void)
{
	...
	Show_Str(30,50,200,16,"KEY2/KEY_UP:连接方式切换",16,0); 	 	
...
u8 sim900a_gprs_test(void)
{
	...
	{
		key=KEY_Scan(0);
		if(key==WKUP_PRES||key==KEY2_PRES)		 
		{  
			mode=!mode;		//连接模式切换
...
void sim900a_mtest_ui(u16 x,u16 y)
{
	...
	Show_Str(x,y+85,200,16,"KEY2/KEY_UP:GPRS测试",16,0);
	POINT_COLOR=BLUE; 	
	USART3_RX_STA=0;
